...
#[wasm_bindgen_test]
async fn compile_compile_error() -> Result<(), JsValue> {
...
#[wasm_bindgen_test]
async fn compile_type_error() -> Result<(), JsValue> {
...
#[wasm_bindgen_test]
async fn compile_valid() -> Result<(), JsValue> {
    let p = WebAssembly::compile(&get_valid_wasm());
    let module = JsFuture::from(p).await?;
