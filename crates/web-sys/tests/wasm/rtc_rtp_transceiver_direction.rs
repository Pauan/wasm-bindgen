...
#[wasm_bindgen_test]
async fn rtc_rtp_transceiver_direction() -> Result<(), JsValue> {
    if !is_unified_avail() {
        return;
    }
...
) -> Result<(RtcPeerConnection, RtcPeerConnection), JsValue> {
    let offer = JsFuture::from(p1.create_offer()).await?;
    let offer = offer.unchecked_into::<RtcSessionDescriptionInit>();
    JsFuture::from(p1.set_local_description(&offer)).await?;
    JsFuture::from(p2.set_remote_description(&offer)).await?;
    let answer = JsFuture::from(p2.create_answer()).await?;
    let answer = answer.unchecked_into::<RtcSessionDescriptionInit>();
    JsFuture::from(p2.set_local_description(&answer)).await?;
    JsFuture::from(p1.set_remote_description(&answer)).await?;
    Ok((p1, p2))
}
